...
import { FileText, Calendar, PoundSterling, Users, TrendingUp, Download, Eye } from 'lucide-react';
    ...
    const fetchInsuranceData = async () => {
      try {
        ...
        const {
          data: companyData,
          error: companyError
        } = await supabase.from('insurance_companies').select('*').eq('active', true).single();
        if (companyError) {
        ...
        const {
          data: entriesData,
          error: entriesError
        } = await supabase.rpc('get_insurance_entries_data', {
        ...
        const {
          data: premiumsData,
          error: premiumsError
        } = await supabase.from('insurance_premiums').select('*').eq('insurance_company_id', companyData.id).order('period_start', {
          ascending: false
        }).limit(12);
          ...
          <div className="max-w-7xl mx-auto space-y-8">
            ...
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-4">
                {company.logo_url && <img src={company.logo_url} alt={company.name} className="w-12 h-12 rounded-lg object-contain bg-white p-2" />}
                <div>
            ...
            <Card>
              ...
              <CardContent>
                <Table>
                  ...
                  <TableBody>
                    {entries.slice(0, 50).map((entry, index) => <TableRow key={index}>
                        <TableCell>
                ...
                {entries.length > 50 && <div className="mt-4 text-center">
            ...
            <Card>
              ...
              <CardContent>
                <Table>
                  ...
                  <TableBody>
                    {premiums.slice(0, 6).map(premium => <TableRow key={premium.id}>
                        <TableCell>
                        ...
                        <TableCell>
                          <Badge variant={premium.status === 'paid' ? 'default' : premium.status === 'approved' ? 'secondary' : 'outline'}>
                            {premium.status}
                          </Badge>
                        </TableCell>
                      </TableRow>)}
