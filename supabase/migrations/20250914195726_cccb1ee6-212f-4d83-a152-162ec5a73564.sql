-- Migration 001: Database Foundation & Schema Enhancements
...
-- Rename timestamp to evidence_captured_at in verifications table
DO $$
BEGIN
  IF EXISTS (SELECT FROM information_schema.columns WHERE table_name='verifications' AND column_name='timestamp') THEN
    ALTER TABLE public.verifications RENAME COLUMN timestamp TO evidence_captured_at;
  END IF;
END
$$;

-- Make entry_id NOT NULL in verifications table
DO $$
BEGIN
...
-- Transfer ownership of helper functions to app_admin and grant permissions
