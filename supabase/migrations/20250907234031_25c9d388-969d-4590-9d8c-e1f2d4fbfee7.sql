-- Temporarily disable the cooldown trigger to allow expired entries to be updated
ALTER TABLE public.entries DISABLE TRIGGER check_entry_cooldown_trigger;

-- Update the function to expire legacy entries after 5 hours instead of 7 days
...
-- Run the function to immediately update any entries that should be expired
SELECT public.update_expired_entries();

-- Re-enable the cooldown trigger
ALTER TABLE public.entries ENABLE TRIGGER check_entry_cooldown_trigger;