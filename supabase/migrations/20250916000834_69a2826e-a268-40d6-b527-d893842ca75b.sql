...
AS PERMISSIVE ON public.clubs
FOR SELECT TO authenticated 
USING (get_current_user_role() = 'ADMIN'::user_role);

CREATE POLICY "admin_select_all_competitions"
AS PERMISSIVE ON public.competitions  
FOR SELECT TO authenticated
USING (get_current_user_role() = 'ADMIN'::user_role);

CREATE POLICY "admin_select_all_entries"
AS PERMISSIVE ON public.entries
FOR SELECT TO authenticated  
USING (get_current_user_role() = 'ADMIN'::user_role);

CREATE POLICY "admin_select_all_verifications"
AS PERMISSIVE ON public.verifications
FOR SELECT TO authenticated
USING (get_current_user_role() = 'ADMIN'::user_role);

CREATE POLICY "admin_select_all_profiles"
AS PERMISSIVE ON public.profiles
FOR SELECT TO authenticated
USING (get_current_user_role() = 'ADMIN'::user_role);