-- Drop all versions of get_public_competition_data to resolve overloading
DROP FUNCTION IF EXISTS public.get_public_competition_data(uuid, text, boolean);
DROP FUNCTION IF EXISTS public.get_public_competition_data(text, text);

-- Create single clean version without demo filtering
...
COMMENT ON FUNCTION public.get_public_competition_data(text, text) IS 'Returns public competition data with club details - no demo filtering';